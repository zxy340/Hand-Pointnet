...
test_wld_err = 0.0
timer = time.time()
with torch.no_grad():  # newly add
	for i, data in enumerate(tqdm(test_dataloader, 0)):
		torch.cuda.synchronize()
		# 3.1 load inputs and targets
		points, volume_length, gt_pca, gt_xyz, images = data
		# gt_pca = Variable(gt_pca, volatile=True).cuda()  # newly delete
		gt_pca = gt_pca.cuda()  # newly add
		...
		# inputs_level1, inputs_level1_center = Variable(inputs_level1, volatile=True), Variable(inputs_level1_center, volatile=True)  # newly delete
