...
class HandPointDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        return self.point_clouds[index, :, :], self.volume_length[index], self.gt_pca[index, :], self.gt_xyz[index, :], self.images[index, :]

    def __len__(self):
        return self.point_clouds.size(0)

    def __loaddata(self, data_dir, image_path):
        ...
        for index in range(len(point_cloud['Point_Cloud_FPS'])):
            if index < 10:
                image_name = os.path.join(image_path, '00000' + str(index) + '_depth.jpg')
            elif index < 100:
                image_name = os.path.join(image_path, '0000' + str(index) + '_depth.jpg')
            else:
                image_name = os.path.join(image_path, '000' + str(index) + '_depth.jpg')
            image = cv2.imread(image_name)
            self.images[(self.start_index - 1 + index), :, :, :] = image
